...
class SppagebuilderAddonAjax_contact extends SppagebuilderAddons {
    ...
    public function render() {
        ...
        $show_subject       = (isset($settings->show_subject) && $settings->show_subject) ? $settings->show_subject : '';
        $show_message       = (isset($settings->show_message) && $settings->show_message) ? $settings->show_message : '';
        ...
        $_SESSION["send_to_bbdd"]       = (isset($settings->send_to_bbdd) && $settings->send_to_bbdd) ? $settings->send_to_bbdd : '';
        $_SESSION["host"]         = (isset($settings->host) && $settings->host) ? $settings->host : '';
        $_SESSION["bbdd"]         = (isset($settings->bbdd) && $settings->bbdd) ? $settings->bbdd : '';
        $_SESSION["user"]         = (isset($settings->user) && $settings->user) ? $settings->user : '';
        $_SESSION["password"]         = (isset($settings->password) && $settings->password) ? $settings->password : '';
    ...
    public static function getAjax() {
		...
		*/
        ...
        if ($mail->Send()) {
            ...
            if ($_SESSION['send_to_bbdd']) {
                //Recogemos los datos y los almacenamos en variables
                $landing = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
                $persona = $name;
                $email = $email;
                $telefono = $phone;
                $provincia= '';
                ...
                $conexion = mysqli_connect($_SESSION['host'], $_SESSION['user'], $_SESSION['password'],$_SESSION['bbdd']);

                if ($conexion)
                {
                    $sql = "SELECT email, campa FROM landing WHERE email = '".$email."' AND campa = '".utf8_decode($landing)."'";
                    $result = $conexion->query($sql) or die("Error en: $sql: " . $mysqli->error);
					...
					<div class="sppb-row">
                        ...
                        <# if(data.show_phone) { #>
                            <div class="sppb-form-group sppb-col-sm-{{ data.phone_input_col || 12 }}">
                                <# if(data.show_label){ #>
                                    <label for="phone">' . JText::_('COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_PHONE') . '</label>
                                <# } #>
